...
# Daily_tech_criteria for daily alerts; RunAway for runaway for active tickers and runaway stocks from weekly list - can use the same output file names
...
def get_tickers_data(conn, tickers_list, datee):
    ...
    postgreSQL_select_Query = "select * from key_indicators_alltickers_sinceapril2023_view where ticker in %(tlist)s and date::date = %(dobject)s"

    if isinstance(datee, datetime.date):
        datee = datee.strftime('%Y-%m-%d')  # Convert to 'YYYY-MM-DD' string format
...
if __name__ == '__main__':
    ...
    # excel_file = r"D:\Trading Dropbox\Satish Udayagiri\SatishUdayagiri\Trading\Process\Daily_Task\daily_work\Daily_Tech_Reading\Weekly_list_temp.xlsx"
    long_file = r"D:\Trading Dropbox\Satish Udayagiri\SatishUdayagiri\Trading\Process\Daily_Task\daily_work\Daily_Tech_Reading\long_file.xlsx"
    short_file = r"D:\Trading Dropbox\Satish Udayagiri\SatishUdayagiri\Trading\Process\Daily_Task\daily_work\Daily_Tech_Reading\short_file.xlsx"
    ...
    columns_to_select = ['ticker','date','high','low','open','close', 'volume', 'vma30','tr', 'dayRange', 'atr14', 'atr_fade_25per','sma10', 'ema20', 'sma50', 'w52high', 'w52low', 'nr7', 'tr3', 'exhaust_trade_bull', 'exhaust_trade_bear', 'runaway_up_521','runaway_down_521', 'runaway_up_1030', 'runaway_down_1030', 'runaway_up_0205', 'runaway_down_0205']
    ...
    long_subset_df = long_subset_df.sort_values(by='ticker')
    short_subset_df = short_subset_df.sort_values(by='ticker')
