...
def create_summary(outputs_df, all_outputs_df, absolute_returns=False):
    ...
    For numeric columns, averages are calculated considering only non-zero values.
    """
    ...
    # Create unique metric names for each statistic
    metric_names = []
    # For Indicator data
    metric_names.extend([f"{int(p * 100)}th Percentile Mean" for p in percentiles])
    metric_names.extend(["Mean", "Median", "Std Dev"])
    # For Baseline data
    metric_names.extend([f"{int(p * 100)}th Percentile Mean (Baseline)" for p in percentiles])
    metric_names.extend(["Mean (Baseline)", "Median (Baseline)", "Std Dev (Baseline)"])

    # Initialize a list of dictionaries, one for each row
    summary_rows = [{'Metric': name} for name in metric_names]

    # Now, for each return column, compute the statistics
    for col in return_columns:
        ...
        # Now, assign the statistics to the corresponding rows
        for idx, value in enumerate(column_summary):
            summary_rows[idx][col] = value

    # Create DataFrame from the list of dictionaries
    summary2_df = pd.DataFrame(summary_rows)
...
def main():
    ...
    if conn is not None:
        tickers = ['SPY','ACIW', 'ALL', 'CAVA' ]  # Replace with your tickers
        ...
        if true_columns_df is not None and not true_columns_df.empty:
            ...
            for idx, row in true_columns_df.iterrows():
                ...
                true_indicators = [col for col in true_columns_df.columns if row[col] == 1]
