## there are 2 versions of this code - which one is correct and need to be retained and the other deleted?
import pandas as pd
import numpy as np
import psycopg2
...
def calculate_runups(ohlc_data: pd.DataFrame):
    ...
    for i in range(1, len(ohlc_data)):
        ...
        if not np.isnan(pivot_low) and pivot_low > 0:
            min_low = pivot_low
            min_low_index = i
            runup_in_progress = False
            peak_index = None
...
def process_ticker_data(ticker, ohlc_data, connection_params):
    ...
    copy_to_postgres(ohlc_data, 'ohlc_intermediate_calculations_v2', connection_params)
    ...
    copy_to_postgres(summary_df, 'ohlc_summary_statistics_v2', connection_params)
...
def create_tables_if_not_exist(connection_params):
        ...
        create_ohlc_intermediate_table_v2 = """
        CREATE TABLE IF NOT EXISTS ohlc_intermediate_calculations_v2 (
            ticker TEXT,
            date DATE,
        ...
        CREATE INDEX IF NOT EXISTS idx_ticker_date ON ohlc_intermediate_calculations_v2(ticker, date);
        """

        # SQL to create the summary statistics table
        create_ohlc_summary_table_v2 = """
        CREATE TABLE IF NOT EXISTS ohlc_summary_statistics_v2 (
        ...
        try:
            ...
            cursor.execute(create_ohlc_intermediate_table_v2)
            cursor.execute(create_ohlc_summary_table_v2)
...
def main():
    ...
    truncate_table('ohlc_intermediate_calculations_v2', connection_params)
    truncate_table('ohlc_summary_statistics_v2', connection_params)
